...
synchronizing the method equals to synchronized block on this. Since thread sync2 is of type Sync2, it won't be able to acquire the lock at all. 
syncl1 and syncl2 threads will be able to enter the synchronized block at the same time. 

If we change the synchronized block to be on Sync1.class, then only one thread (of the 3 above) will be called. Thus, a possible output will be then:
2
2
2
2
